...
**No formula internals in UI:** the user does not want Qi Men formula mechanics surfaced to end users (e.g. per-variant descriptions like "используется без учёта Главных Врат", "не Огонь Инь… позитивный элемент или Мистик", "самый сильный вариант"). These were removed from both the result cards and the info dialog, and the `variantText`/`variant` fields were dropped from the QimenJadeMaiden API entirely. Keep future esoteric structures user-facing = purpose/usage + sector/time only; formula rationale stays in code comments.
