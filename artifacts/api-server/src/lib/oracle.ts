...
const MATRIX_ARCANA_THEMES: Record<number, string> = {
  1: "Аркан призывает опираться на инициативу, личное решение и способность начать дело.",
  2: "Аркан раскрывается через наблюдательность, тонкое чувство момента и внимательное отношение к внутреннему голосу.",
  3: "Аркан усиливает созидание, заботу, творческий подход и умение взращивать то, что для Вас ценно.",
  4: "Аркан просит выстроить порядок, обозначить границы и взять ответственность за выбранную опору.",
  5: "Аркан выделяет ценности, знания, наставничество и поиск собственных нравственных ориентиров.",
  6: "Аркан ставит в центр зрелый выбор, честный диалог и отношения, в которых есть место обоим людям.",
  7: "Аркан поддерживает движение к цели, дисциплину, собранность и умение держать выбранный курс.",
  8: "Аркан напоминает о спокойной силе, выдержке и способности направлять энергию без давления.",
  9: "Аркан предлагает замедлиться, осмыслить опыт и услышать ответ в тишине, а не в спешке.",
  10: "Аркан обращает внимание на смену циклов, гибкость и готовность заметить вовремя появившуюся возможность.",
  11: "Аркан учит соразмерности, честной оценке обстоятельств и ответственности за последствия решения.",
  12: "Аркан предлагает сделать паузу, отпустить привычный угол зрения и дать ситуации раскрыться по-новому.",
  13: "Аркан связан с завершением отжившего, очищением пространства для нового этапа и готовностью к обновлению.",
  14: "Аркан помогает соединять противоположности, выбирать спокойный ритм и восстанавливать внутреннее равновесие.",
  15: "Аркан просит честно смотреть на желания, ресурсы и привязанности, сохраняя свободу собственного выбора.",
  16: "Аркан показывает, где прежняя конструкция стала тесной и где нужна смелость перестроить основу.",
  17: "Аркан раскрывает вдохновение, творческое видение и способность поддерживать надежду конкретными действиями.",
  18: "Аркан усиливает чувствительность и интуицию, одновременно предлагая отличать переживания от проверенных фактов.",
  19: "Аркан поддерживает открытое проявление, радость от результата и право быть заметным в своём деле.",
  20: "Аркан связан с переоценкой прошлого, прояснением важных решений и возвращением к тому, что действительно имеет значение.",
  21: "Аркан помогает завершить большой цикл, собрать опыт в целое и увидеть более широкий горизонт.",
  22: "Аркан открывает пространство для нового опыта, смелого первого шага и разумного эксперимента.",
};
...
> = {
  day: {
    ...
    essence: "Личная точка входа в матрицу. Она показывает, с какой внутренней позицией Вы проявляете себя, принимаете решения и заявляете о своих потребностях.",
  },
  month: {
    position: "Месяц рождения",
    essence: "Верхняя точка диагонального квадрата. Она связана с тем, как Вы воспринимаете опыт, учитесь, ищете смысл и опираетесь на свои ценности.",
  },
  year: {
    position: "Редуцированный год",
    essence: "Левая точка земной оси. Она показывает, как накопленный опыт проявляется в Ваших делах, окружении и практической стороне жизни.",
  },
  foundation: {
    position: "Нижняя опора",
    essence: "Точка соединяет день, месяц и год. Она описывает задачу воплощения: как соединить личные качества, внутренние ориентиры и жизненный опыт в реальных действиях.",
  },
  center: {
    position: "Зона комфорта",
    essence: "Зона комфорта показывает, что помогает Вам восстанавливаться, чувствовать внутреннюю опору и действовать в согласии с собой. В популярных калькуляторах её также называют центральной энергией или ядром матрицы.",
  },
  directTopLeft: {
    position: "Род матери: духовная сфера",
    essence: "Эта точка относится к материнской линии в духовной сфере: ценностям, знаниям, отношениям, словам и убеждениям, которые могли передаваться в семье. Она задаёт направление для изучения ресурсов и задач этой ветви рода.",
  },
  directTopRight: {
    position: "Род отца: духовная сфера",
    essence: "Эта точка относится к отцовской линии в духовной сфере: ценностям, знаниям, отношениям, словам и убеждениям, которые могли передаваться в семье. Она задаёт направление для изучения ресурсов и задач этой ветви рода.",
  },
  directBottomRight: {
    position: "Род матери: материальная сфера",
    essence: "Эта точка относится к материнской линии в материальной сфере: действиям, быту, личным границам и обращению с ресурсами. Её рассматривают вместе с духовной точкой линии матери.",
  },
  directBottomLeft: {
    position: "Род отца: материальная сфера",
    essence: "Эта точка относится к отцовской линии в материальной сфере: действиям, быту, личным границам и обращению с ресурсами. Её рассматривают вместе с духовной точкой линии отца.",
  },
  heaven: {
    position: "Линия Неба",
    essence: "Линия Неба складывается из месяца и нижней опоры. Она описывает внутренний потенциал, запас энергии для развития и то, на что Вы опираетесь в поиске смысла.",
  },
  earth: {
    position: "Линия Земли",
    essence: "Линия Земли складывается из дня и года. Она относится к практической и материальной стороне жизни: действиям, устойчивости, быту и умению воплощать идеи.",
  },
  personalPurpose: {
    position: "Личное предназначение",
    essence: "Личное предназначение объединяет Небо и Землю. Это вопрос о том, что важно понять, развить и изменить в себе для более цельного жизненного пути.",
  },
  fatherLine: {
    position: "Род отца",
    essence: "Линия рода отца собирает две отцовские опоры. Она предлагает бережно посмотреть на унаследованные способы действовать, защищать себя и принимать поддержку.",
  },
  motherLine: {
    position: "Род матери",
    essence: "Линия рода матери собирает две материнские опоры. Она предлагает бережно посмотреть на унаследованные способы строить близость, заботиться и делать выбор.",
  },
  socialPurpose: {
    position: "Социальное предназначение",
    essence: "Социальное предназначение объединяет две родовые линии. Оно показывает, какие качества, знания или опыт Вы можете развивать и передавать людям во внешнем мире.",
  },
  spiritualPurpose: {
    position: "Духовное предназначение",
    essence: "Духовное предназначение соединяет личную и социальную задачи. Оно помогает увидеть общий вектор роста: как согласовать внутренние ценности с пользой, которую Вы хотите приносить другим.",
  },
  planetaryPurpose: {
    position: "Планетарное предназначение",
    essence: "Планетарное предназначение показывает самый широкий масштаб Вашего вклада. Это тема ценности, которую Вы способны создавать для команды, сообщества или более широкого круга людей.",
  },
};
  ...
  return {
    ...
    points: FULL_MATRIX_POINT_ORDER.map((id) => {
      ...
      return {
        ...
        essence: `${copy.essence} ${MATRIX_ARCANA_THEMES[arcana.number]}`,
